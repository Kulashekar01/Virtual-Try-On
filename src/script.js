...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleFile(file, index) {
        ...
        if (file && file.type.startsWith('image/')) {
            ...
            const uploadBoxId = index === 0 ? 'personUpload' : 'garmentUpload';
            ...
            reader.onload = (e) => {
                ...
                uploadBox.classList.add('has-image'); // Hide the upload box
            };
    ...
    // Add click handler for previews to allow re-upload
    document.querySelectorAll('.preview').forEach((preview, index) => {
        preview.addEventListener('click', () => {
            const uploadBox = document.getElementById(index === 0 ? 'personUpload' : 'garmentUpload');
                ...
                if (imageData.image && imageData.image.url) {
                    resultBox.innerHTML = `
                        <div class="result-wrapper">
                            <a href="${imageData.image.url}" download class="download-btn">
